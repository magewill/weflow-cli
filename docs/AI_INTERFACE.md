...
The local daily reader follows the same lifecycle pattern. `daily-server --status --json` only checks loopback status. Starting it requires `daily-server --dry-run --json` followed by user-approved `daily-server --yes --json`; add `--open` only when the user also wants a browser window opened. Human-oriented non-JSON startup remains compatible. The older `fav-server` command is a compatibility entry and applies the same machine-mode confirmation rule.
